...
#undef expr
  ...
  */
  ...
  inline Scalar max_element()   // cjren, the original name is max, but conflict with the same name in windef.h
  {
  ...
  inline Scalar min_element() const // cjren, min
  {
    Scalar m(Base::values_[0]);
    for(int i=1; i<DIM; ++i) if(Base::values_[i]<m) m=Base::values_[i];
    return m;
...
#undef expr
  ...
  /// component-wise min
  inline vector_type min_compoent_wise(const vector_type& _rhs) { // cjren, min, name conflict
    return vector_type(*this).minimize(_rhs);
  }

  /// component-wise max
  inline vector_type max_compoent_wise(const vector_type& _rhs) { // cjren, max, name conflict.
